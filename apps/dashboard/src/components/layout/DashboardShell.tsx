  ...
  return (
    <div className="min-h-screen bg-[var(--np-canvas)] text-sm">
      ...
      <div className="lg:pl-[248px]">
        <header className="sticky top-0 z-20 flex h-17 items-center justify-end border-b border-[var(--np-line)] bg-white/95 px-4 backdrop-blur sm:px-6 lg:px-8"><div className="mr-auto lg:hidden"><Brand compact /></div><div className="flex items-center gap-2"><button className="relative grid size-9 place-items-center rounded-[9px] text-[var(--np-muted)] hover:bg-slate-100" aria-label="Notifications"><AlarmIcon size={19}/><span className="absolute right-1.5 top-1.5 size-2 rounded-full bg-[var(--np-critical)] ring-2 ring-white"/></button><button onClick={() => setMobileNav((open) => !open)} className="grid size-9 place-items-center rounded-[9px] text-[var(--np-muted)] hover:bg-slate-100 lg:hidden" aria-label="Open navigation"><ListMagnifyingGlassIcon size={20}/></button><span className="hidden h-8 w-px bg-[var(--np-line)] sm:block"/><div className="hidden items-center gap-2 sm:flex"><span className="grid size-8 place-items-center rounded-full bg-[var(--np-brand)] text-xs font-medium text-white">AH</span><span className="font-medium">Agus</span></div></div></header>
